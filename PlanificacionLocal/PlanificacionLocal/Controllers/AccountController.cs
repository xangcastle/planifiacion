    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //Modificado para guardar usuario
                    var dato = await UserManager.FindAsync(model.UserName, model.Password);
                    Usuario oUsua = new Usuario();
                    oUsua.NombreDeUsuario = dato.UserName;
                    oUsua.ClaveDeUsuario = dato.PasswordHash;
                    db.Usuarios.Add(oUsua);
                    db.SaveChanges();
