...
var dayIDs = [ 'do', 'lun', 'jue', 'mie', 'jue', 'vie', 'sab' ];
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '" title="Dar click para visualizar en detalle la actividad."' +
				(event.url ?
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					td = $('<td class="fc-event-container" title="Dar un click para modificar la actividad seleccionada &oacute; mantega presionado el mouse y mueva a una fecha distinta" />').append(seg.el);
...
DayGrid.mixin({
	...
	renderMoreLink: function(cell, hiddenSegs) {
		...
		return $('<a class="fc-more" title="Dar click para listar las actividades correspondiente a la fecha."/>')
			.text(
...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	if ($.datepicker) {
		...
		$.datepicker.regional.en = $.datepicker.regional['es-NI'];
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
					    groupChildren = groupChildren.add($('<h2 id="TituloMesId">&nbsp;</h2>')); // we always want it to take up height
					    isOnlyButtons = false;
	...
	numberCellHtml: function(cell) {
		...
		if ($("#AbilitaLlave").val() == "true") {
			...
			'<td class="' + classes.join(' ') + '" data-date="' + date.format() + '">' +
				date.date() + '<input type="checkbox" id="All" name="' + "NM" + date.format('MMMM YYYY') + '" disabled="disabled" class="ocultar" title="Checkear el checkbox para seleccionar la fecha para la actividad" data-FechR="' + date.format('DD/MM/YYYY') + '" value="' + date.format('DD/MM/YYYY') + '">'
            + '</td>';
		} else {
		    return '' +
			'<td class="' + classes.join(' ') + '" data-date="' + date.format() + '">' +
...
function MuestraCheckbox() {
    $('input[name="' + "NM" + $("#TituloMesId").text().trim() + '"]').removeAttr('disabled','disabled');
    $('input[name="' + "NM" + $("#TituloMesId").text().trim() + '"]').removeClass('ocultar');
}