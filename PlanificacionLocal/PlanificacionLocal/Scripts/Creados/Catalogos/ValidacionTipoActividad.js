...
function keypressDescripcion(n) {
    return (tecla = document.all ? n.keyCode : n.which, tecla == 8) ? !0 : tecla == 0 ? !0 : (patron = /[a-zA-Zá-úÁ-Ú0-9/.#,\- ]/,
        te = String.fromCharCode(tecla), patron.test(te))
}

function MsjDanger(n, t) {
    $(".loading_body").fadeOut(300);
    $(t).removeClass("alert-success alert-info alert-warning alert-danger").addClass("alert-danger");
    $(t).html("<i class='icon2-cancel-circle'><\/i>" + (n != null && n != "" ? n : "Su operación no pudo ser realizada, inténtelo mas tarde."));
    $(t).show()
}